(function () {
	...
	document.getElementById("Dnode").addEventListener('click', function () {
		...
		for (var i = graphAdge.length - 1; i >= 0; i--) {
			if ((graphAdge[i].start == node) || (graphAdge[i].end == node)) {
	...
	var GraphList = function (graphVertex, graphAdge) {
		...
		var dfslist = function (v) {
			let result2 = [];
			result2 = dfsTraverse(v);
			for (var k = 0; k < vertex.length; k++) {
				var test = dfsTraverse(vertex[k]);
				...
				// result2.push.apply(result2, test);
			}
			showGraph();
			return result2;
			console.log(result);
		...
		var bfslist = function (v) {
			let result2 = [];
			result2 = bfsTraverse(v);
			for (var k = 0; k < vertex.length; k++) {
				var test = bfsTraverse(vertex[k]);
	...
	var showGraph = function () {
		...
		for (let i = 0; i < adge.length; i++) {
			...
			arrowStr += '<polygon points="' + data.points + '" fill:"#131917" transform="rotate(' + angle +
				')"/>';
	...
	console.log("基于邻接表实现：");
	// console.log("深度优先遍历", graph.dfslist("v2"));
	...
	button.addEventListener("click", function () {
		...
		switch (index) {
			case 0:
				traversalArr = graph.dfslist(selectValue);
				var str = '<h3>深度优先遍历</h3>';
				$(".mytitle").html(str);
				var dfs = graph.dfslist(selectValue);
				...
				for (var i = 0; i < dfs.length; i++) {
					...
					ul_node.children[i].style.display = "inline-block";
				}
				break;
			case 1:
				traversalArr = graph.bfslist(selectValue);
				var str = '<h3>广度优先遍历</h3>';
				$(".mytitle").html(str);
				var bfs = graph.bfslist(selectValue);
				if ($("ul.graph_node").find("li").length > 0) {
					var childs = ul_node.childNodes;
					console.log(childs)
					for (var i = childs.length - 1; i >= 0; i--) {
